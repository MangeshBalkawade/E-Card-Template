const DbModel = require("../models/cityModel");
const logService = require("./logService");
const tableName = DbModel.tableName;
const PrimaryKeyID = "cityId";
...
module.exports = {
  ...
  async bulkCreateData(data) {
    try {
      ...
      await commonUtils.log(error, "Error");
      throw new Error("Unable to bulk create data");
    }
  },
  ...
  async getAllWithPagination(filter, page, PageSize, orderBy, orderDir) {
    try {
      ...
      searchFields = searchFields
        ? JSON.parse(searchFields.replace(/'/g, '"'))
        : [];
      ...
      searchByColumNameValue = searchByColumNameValue
        ? JSON.parse(searchByColumNameValue.replace(/'/g, '"'))
        : {};
      ...
      if (Object.keys(searchByColumNameValue).length > 0) {
        for (const column in searchByColumNameValue) {
          ...
          whereClause[column] = {
            [Op.like]: `%${searchByColumNameValue[column]}%`,
          };
        }
      }
  ...
  async getDataById(id) {
    try {
      ...
      // Log the error and throw an exception
      await commonUtils.log(error, "Error");
      await logModel.Insert({ id, stack: error.stack }, error);
      throw new Error(Messages.UnableToGetEntity);
    }
  },
  ...
  async updateData(id, updatedData) {
    try {
      ...
      await commonUtils.log(error, "Error");
      await logModel.Insert({ id, updatedData, stack: error.stack }, error);
  ...
  // Function to get filtered data by organizationId and order
  async getFilterSingleOneDataWithJoin(
    filter,
    orderBy = "createdAt",
    order = "ASC"
  ) {
